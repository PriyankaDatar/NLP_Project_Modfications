...
def epoch_train(model, optimizer, batch_size, sql_data, table_data, pred_entry):
    ...
    while st < len(sql_data):
        ...
        loss = model.forward(q_seq, col_seq, col_num, pred_entry,ans_seq,optimizer,
                gt_where=gt_where_seq, gt_cond=gt_cond_seq, gt_sel=gt_sel_seq,is_train=True)
...
def epoch_acc(model, batch_size, sql_data, table_data, pred_entry):
    ...
    while st < len(sql_data):
        ...
        gt_sel_seq = [x[1] for x in ans_seq]
        score = model.forward(q_seq, col_seq, col_num,
                pred_entry,ans_seq,None, gt_sel = gt_sel_seq,is_train=False)
        pred_queries = model.gen_query(score, q_seq, col_seq,
